...
vector<int> MiniMax_DFS::minValue(vector<vector<int> >& board, int previousRow, int previousCol, int alpha, int beta, int depth)
{
	int t = HeuristicFunction::terminalBoard(board);
	if (abs(t) == MAX) return {t, previousRow, previousCol};
	if (cutoffTest(board, depth, INF)) return {heuristicFunction.execute(board), previousRow, previousCol};
...
vector<int> MiniMax_DFS::maxValue(vector<vector<int> >& board, int previousRow, int previousCol, int alpha, int beta, int depth)
{
	int t = HeuristicFunction::terminalBoard(board);
	if (abs(t) == MAX) return {t, previousRow, previousCol};
	if (cutoffTest(board, depth, INF)) return {heuristicFunction.execute(board), previousRow, previousCol};
